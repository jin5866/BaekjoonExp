...
int main()
{
	...
	std::vector<std::vector<int>> maxs(n, std::vector<int>(n,0));
	...
	for (int i = 1; i < n; i++) {
		for (int j = 0; j < n - i; j++) {
			...
			for (int mid = 0; mid < i; mid++) {
				minnum = std::min(minnum, maxs[j][j + mid] + maxs[j + mid + 1][j + i] + sizes[j].first * sizes[j + mid].second * sizes[j + i].second);
			}
